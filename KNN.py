"""
ECG KNN Classifier v6 — MIT-BIH + svdb, AAMI 3-class + timing instrumentation
=============================================================================
Changes from v5 (review corrections):
  - [FIX] K=1 excluded from hyperparameter grid (start k=3) — k=1 risks
    memorising SMOTEENN synthetic samples rather than learning a real
    decision boundary, making CV scores unreliable
  - [FIX] weights="distance" added — closer neighbours cast stronger
    votes, improving minority-class (S) sensitivity
...
SEED         = 42
OUTPUT_DIR   = "./outputs"
MODEL_PATH   = f"{OUTPUT_DIR}/ecg_knn_3class_v6.pkl"
RESULTS_JSON = f"{OUTPUT_DIR}/results_knn_3class_v6.json"
...
with Timer("data_loading_total"):
    with Timer("load_mitdb"):
        print(f"\n Loading MIT-BIH ({len(MITDB_RECORDS)} records, paced excluded)…")
...
# ════════════════════════════════════════════════════════════
# STEP 7 — KNN GRIDSEARCH
# ════════════════════════════════════════════════════════════
print("\n" + "═" * 60); print(" STEP 7 — KNN GRIDSEARCH (5-fold CV)"); print("═" * 60)
# K=1 excluded: risks memorising SMOTEENN synthetic samples instead of
# learning a real decision boundary — CV scores at k=1 are inflated.
# weights="distance": closer neighbours cast stronger votes,
# which improves sensitivity on minority classes (S, V).
k_grid = list(range(3, 22, 2))
param_grid = {"n_neighbors": k_grid}
print(f" Grid: k ∈ {k_grid} ({len(k_grid)} values) × 5-fold = {len(k_grid)*5} fits")
with Timer("gridsearch_knn"):
    knn_base = KNeighborsClassifier(metric="euclidean", weights="distance", n_jobs=-1)
    cv = StratifiedKFold(n_splits=5, shuffle=True, random_state=SEED)
    grid_search = GridSearchCV(knn_base, param_grid, cv=cv,
...
with Timer("final_training"):
    ...
    knn_final.fit(X_tr_bal, y_tr_bal)

# ════════════════════════════════════════════════════════════
# STEP 9 — EVALUATION + LATENCY
# ════════════════════════════════════════════════════════════
print("\n" + "═" * 60); print(" STEP 9 — TEST EVALUATION + LATENCY"); print("═" * 60)
with Timer("prediction_total"):
    y_pred = knn_final.predict(X_te_pca)
n_test = len(y_test)
...
# KNN can be slow per-call; use 200 iterations for the single-beat benchmark
n_single_runs = 200
t0 = time.perf_counter()
for _ in range(n_single_runs):
    _ = knn_final.predict(single_beat)
...
joblib.dump({
    "scaler": scaler, "pca": pca, "knn": knn_final, "label_enc": le,
    "best_k": best_k, "window": WINDOW_TARGET, "fs": FS_TARGET,
...
training_phase_keys = ["data_loading_total", "preprocessing", "feature_extraction",
                       "scaling_pca", "smoteenn", "gridsearch_knn", "final_training"]
...
plt.title("Confusion Matrix — KNN, AAMI 3-class: N, S, V (MIT-BIH + svdb)")
plt.savefig(f"{OUTPUT_DIR}/cm_knn_3class_v6.png", dpi=150, bbox_inches="tight")
plt.close()
print(f" Confusion matrix : {OUTPUT_DIR}/cm_knn_3class_v6.png")
